...
App.views.PersonasOrganizacionesMainView = Backbone.View.extend({
	...
	initialize : function() {
		...
		this.filtrarPersonas([{"filtro" : "limit",	valor : 10}]);
		...
		//primera carga ejecutar la paginación
		//this.personasListView.paginaSeleccionada=1;
		//this.personasListView.paginacion();
				...
				jConfirm(mensaje, 'Mensaje de confirmación', function(r) {
					if (r) {
						...
						if (tipoPersona == "natural"){
								...
								self.model.organizaciones.fetch({
													async:false,
													success: function(model){
														if(model.length) {
														      mensaje = mensaje + "<li>"+plcMensajes.persona.msjAvisoNOeliminar_tiene_Organizaciones+"</li>";
								...
								self.model.miembros.fetch({
													async:false,
													success: function(model){
														if(model.length) {
															mensaje = mensaje + "<li>"+plcMensajes.persona.msjAvisoNOeliminar_tiene_Miembro+"</li>";
															eliminado = false;
														}
													} 
								});
					...
					if(eliminado){	
						...
						if (tipoPersona == "natural") mensajeParaMantenedor(plcMensajes.persona.msjNOeliminacion+"<ul>  " + mensaje + " </ul>", 'alert-danger');
		 				else mensajeParaMantenedor(plcMensajes.organizacion.msjNOeliminacion+"<ul> " + mensaje + " </ul>", 'alert-danger');
					}
...
App.views.PersonaLocalizacionView = Backbone.View.extend({
	...
	events : {
		...
		"click .btn.lat:": "cambiarCoordenadas",
		"click .btn.lon:": "cambiarCoordenadas"
	},
	...
	cambiarCoordenadas:function(){
			var latitud = $("#latitud").val();
			var longitud = $("#longitud").val();
			if(latitud!="" && longitud !="")
				initializeMapa(latitud,longitud);
	},
	eliminar : function() {
